...
#include "../headers/Upgrade_tablet.h"
...
void Upgrade_tablet::create_upgrade(buildings *resp, std::string caption, sf::Vector2f pos){
    ...
    this->star_text.create(DEFAULT_FONT,width+41,this->position.y+10,std::to_string(this->responsible->get_level()),25);
    ...
    this->counting_down.setPosition(this->position+sf::Vector2f(300,0));
...
void Upgrade_tablet::set_position(sf::Vector2f new_pos){
    ...
    this->star_text.setPosition(sf::Vector2f(this->position.x+this->name.getWidth()+41,this->position.y+10));//star text
}
...
int Upgrade_tablet::update_timer(float time_passed){
    if(this->responsible->status()){
        ...
        //update progress bar
        sf::Vector2f size = this->counting_down.getSize()-sf::Vector2f(time_passed * (30.f/((float) this->responsible->get_level()+1)), 0);
