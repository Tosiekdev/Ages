...
void HandleGame::create(std::string path){
    ...
    resourceMan_.create_obstacle("Textures/human.png", sf::Vector2f(0.2, 0.2),
                                 sf::Vector2f(570,5));
    resourceRock_.create_obstacle("Textures/rock.png", sf::Vector2f(0.32, 0.32),
                                  sf::Vector2f(570, 41));
    resourceWood_.create_obstacle("Textures/wood.png", sf::Vector2f(0.1, 0.1),
                                  sf::Vector2f(570, 77));
    resourceMoney_.create_obstacle("Textures/wood.png", sf::Vector2f(0.1, 0.1),
                                  sf::Vector2f(570, 113));

    //resource numbers
    lHuman_.create(DEFAULT_FONT, 620, 5, std::to_string(human_), 24);
    lRock_.create(DEFAULT_FONT, 620, 41, std::to_string(rock_), 24);
    lWood_.create(DEFAULT_FONT, 620, 77, std::to_string(wood_), 24);
    lMoney_.create(DEFAULT_FONT, 620, 113, std::to_string(money_), 24);
}
    ...
    while(window.pollEvent(this->e_)){
        ...
        if(sf::Mouse::isButtonPressed(sf::Mouse::Left)){
            ...
            if(this->building_[6].isClicked(i, j)){
                this->thWindow_.create(&lHuman_,&lRock_,&lWood_,&lMoney_,&human_,
                                       &rock_,&wood_,&money_,&levels_[6]);
                scene=5;
            }
...
void HandleGame::prepare_buildings(){
    building_[0].create_building("Academy", "Textures/academy.png", this->levels_[0],
                                       sf::Vector2f(250,300), 100, 75, 10);
    building_[1].create_building("Barracks", "Textures/barracks.png", this->levels_[1],
                                       sf::Vector2f(50,300), 75, 75, 7);
    building_[2].create_building("Church", "Textures/church.png", this->levels_[2],
                                       sf::Vector2f(300,150), 50, 150, 10);
    building_[3].create_building("Farm", "Textures/farm.png", this->levels_[3],
                                       sf::Vector2f(600,450), 20, 60, 2);
    building_[4].create_building("Lumber mill", "Textures/lumbermill.png", this->levels_[4],
                                       sf::Vector2f(400,450), 65, 15, 2);
    building_[5].create_building("Stone pit", "Textures/default.png", this->levels_[5],
                                       sf::Vector2f(600,250), 15, 65, 2);
    building_[6].create_building("Town Hall", "Textures/town_hall.png", this->levels_[6],
                                       sf::Vector2f(100,100), 75, 75, 12);
    building_[7].create_building("Warehouse", "Textures/magazine.png", this->levels_[7],
...
void HandleGame::clocks(int scene){
    if(scene>3){
        ...
        for(int i=0; i<9; i++){
            if(this->thWindow_.ub_get_time(i) >= 1){
                if(this->thWindow_.ub_update_timer(i, this->thWindow_.ub_get_time(i)) == 3)
                    this->assign_levels();
                this->thWindow_.ub_reset_timer(i);
            }
        }
    }
}
