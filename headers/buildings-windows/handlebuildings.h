...
public:
    ...
    create(Label *lh, Label *lr, Label *lw, Label *lm, Label *li, int *hn, int *rk, int *wd, int *moni, int *iron,
           int *bld) =0;
    void main_loop(sf::Event &event, sf::RenderWindow &window, buildings::Scene &scene);
    virtual void handle_events(sf::Event &event, sf::RenderWindow &window, buildings::Scene &scene)=0;
    ...
    void crete_info();
    void return_to_village(buildings::Scene &scene);
